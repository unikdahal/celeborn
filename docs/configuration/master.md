...
| celeborn.master.recovery.blob.maxPayloadSize | 64m | false | Largest recovery payload a worker will store as a blob. This bounds worker disk per record rather than master heap, so it is far larger than the inline payload limit. | 1.0.0 |  | 
